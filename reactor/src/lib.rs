...
impl<R: Resource, M: ResourceMgr<R>, H: Handler<R>> Runtime<R, M, H> {
    ...
    fn dispatch(&mut self) -> Result<(), R::Error> {
        ...
        // TODO: Move it back since it breaks the order of [`Handler`] calls.
        for (remote_raw, direction) in new_connections {
            let resource = self.handler.handshake(remote_raw)?;
...
/// Input events generated by the resources monitored in the reactor.
#[derive(Clone, Ord, PartialOrd, Eq, PartialEq, Hash, Debug)]
pub enum InputEvent<R: Resource> {
    /// A raw connection to the resource was successfully established.
    ///
...
pub trait ResourceMgr<R: Resource>: Send {
    ...
    ///
    /// Does not cover raw resource connections.
    fn get(&self, addr: &R::Addr) -> Option<&R>;

    /// Checks if the resource is in the process of connection (i.e. after
    /// [`Self::connect_raw`] but before [`Self::register_resource`] is called).
    fn is_connecting(&self, addr: &R::Addr) -> bool;

    /// Connects raw resource and track raw connection until the handshake is
    /// completed and [`Self::register_resource`] is called.
    ///
    /// The implementations must make sure that [`InputEvent::RawConnected`] is
    /// emit upon successful connection.
    ///
    /// Return false if the resource is already connected or in a connection
    /// process.
    fn connect_raw(&mut self, addr: &R::Addr, direction: ConnDirection) -> Result<bool, R::Error> {
        if self.has(addr) || self.is_connecting(&addr) {
            return Ok(false);
        }
        let raw = R::raw_connection(addr)?;
        self._register_raw(raw, direction);
        Ok(true)
    }

    /// Adds already operating/connected resource to the manager after the
    /// handshake procedure. Returns `None` if the resource is already known,
    /// or a reference to the added resource.
    ///
    /// The implementations should not emit [`InputEvent`].
    fn connect_resource(&mut self, resource: R) -> Option<&R>;

    /// Disconnects resource and removes it from the manager.
    ///
    /// The implementations must make sure that [`InputEvent::Disconnected`] is
    ...
    /// Adds raw resource with existing connection to the manager. Returns false
    ...
    ///
    /// Should not produce any [`InputEvent`].
