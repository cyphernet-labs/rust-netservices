...
use streampipes::{HandshakeMgr, Resource};
            ...
            match event {
                InputEvent::Connected {
                    ...
                    let resource = self
                        .resources
                        .get(&remote_addr)
                        .expect("broken resource manager");
                    self.handler.connected(resource, direction)
                }
...
pub trait ResourceMgr<R: Resource>: HandshakeMgr<R> + Send {
