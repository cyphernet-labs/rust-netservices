...
impl<R: Resource, M: ResourceMgr<R>, H: Handler<R>> Runtime<R, M, H> {
    ...
    fn dispatch(&mut self) -> Result<(), R::Error> {
        let events = self.resources.try_read_events()?;
        self.handler.tick();
        for event in events {
            match event {
...
pub trait Handler<R: Resource>: Send {
    ...
    fn disconnected<DisconnectReason>(&mut self, remote_addr: R::Addr, reason: DisconnectReason);
...
pub enum InputEvent<R: Resource, DisconnectReason> {
    ...
    Disconnected(R::Addr, DisconnectReason),
...
pub trait ResourceMgr<R: Resource>: Send {
    /// Iterator over input events returned by [`Self::try_read_events`].
    type EventIterator: Iterator<Item = InputEvent<R, Self::DisconnectReason>>;
    ...
    /// Connects new resource and adds it to the manager.
    ///
    /// The implementations must make sure that [`IoEvent::Connected`] is emit
    ...
    /// Returns an iterator over newly generated events from all resources under
    /// this manager. May fail.
